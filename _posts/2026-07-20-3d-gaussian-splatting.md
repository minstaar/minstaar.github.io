...
Radiance field 기반 방법들은 여러 장의 사진이나 영상으로 촬영한 장면의 novel-view synthesis를 크게 발전시켰다. 하지만 높은 visual quality를 얻으려면 학습과 렌더링 비용이 큰 neural network가 필요하고, 최근의 빠른 방법들은 speed를 위해 quality를 희생하는 경향이 있다. 특히 unbounded하고 완전한 장면(고립된 물체가 아닌)에 대해 1080p 해상도로 렌더링할 때, 실시간(real-time) display rate을 달성하는 방법은 아직 없었다.
...
1. 3D Gaussian을 flexible하고 expressive한 scene representation으로 도입한다. Structure-from-Motion(SfM)으로 얻은 sparse point에서 시작하며, 대부분의 point-based 방법과 달리 continuous volumetric radiance field의 좋은 성질을 보존하면서도 불필요한 빈 공간(empty space) 계산을 피한다.
2. 3D Gaussian에 대한 optimization과 adaptive density control을 번갈아 수행한다. 특히 anisotropic covariance를 최적화하여 장면을 정확하게 표현한다.
...
Neural Radiance Fields(NeRF)는 MLP로 장면을 표현하고 volumetric ray-marching으로 렌더링하여 높은 품질을 달성했다. 하지만 원래 NeRF는 학습과 렌더링이 매우 느리다. 이후 Mip-NeRF360은 quality 측면에서 SOTA를 달성했으나 학습에 최대 48시간이 걸린다. 반면 InstantNGP는 hash grid를, Plenoxels는 sparse voxel grid를 사용하여 학습을 크게 가속했지만, unbounded 장면의 최고 품질에는 미치지 못하거나 real-time rendering이 어렵다.

### Point-Based Rendering and Radiance Fields
Point-based 방법은 GPU에서 효율적으로 렌더링되지만, point sampling에 따른 hole이나 aliasing 문제를 겪는다. Differentiable point-based rendering은 이러한 point를 최적화 가능한 primitive로 다루며, 본 논문의 3D Gaussian은 이 계열을 발전시킨 것이다. 3D Gaussian은 미분 가능하고 rasterize하기 쉬우면서도, continuous representation의 장점을 유지한다.
...
핵심 문제는 covariance matrix $\Sigma$를 직접 최적화하면, physically meaningful하려면 positive semi-definite해야 하는데 gradient descent로는 이 제약을 쉽게 보장할 수 없다는 점이다. 따라서 저자들은 $\Sigma$를 직접 다루는 대신, 타원체(ellipsoid)의 configuration을 나타내는 더 직관적이면서 동등하게 표현력 있는 형태로 분해한다. 즉, scaling matrix $S$와 rotation matrix $R$을 사용하여 다음과 같이 covariance를 구성한다.
...
_Fig 4. Adaptive Gaussian densification 방식. 위 행(under-reconstruction)에서는 small-scale geometry가 충분히 표현되지 않은 경우 해당 Gaussian을 clone한다. 아래 행(over-reconstruction)에서는 하나의 Gaussian이 넓은 영역을 덮는 경우 이를 split한다._
...
Densification은 view-space positional gradient의 크기가 임계값 $\tau_{\text{pos}}$(= 0.0002)를 넘는 Gaussian을 대상으로 한다. 이는 under-reconstruction과 over-reconstruction 두 경우를 모두 다룬다.

- Under-reconstruction (missing geometry): 작은 Gaussian이 있는 영역에서는 같은 크기의 Gaussian을 clone하여 positional gradient 방향으로 이동시킨다. 이는 전체 volume과 Gaussian 수를 늘린다.
- Over-reconstruction (하나의 Gaussian이 넓은 영역을 덮는 경우): 큰 Gaussian을 두 개의 작은 Gaussian으로 split한다. 이때 scale을 실험적으로 정한 factor $\phi = 1.6$으로 나눈다.

또한 floater(입력 카메라 근처에 떠 있는 불필요한 Gaussian)가 과도하게 증식하는 것을 막기 위해, 3000 iteration마다 opacity $\alpha$를 0에 가깝게 reset한다. 이후 optimization이 필요한 Gaussian의 $\alpha$를 다시 키우고, 그렇지 않은 것은 prune된다.

## Fast Differentiable Rasterizer for Gaussians

빠른 렌더링과 sorting을 위해, 임의 개수의 blended Gaussian에 대해 gradient를 계산할 수 있는 tile-based rasterizer를 설계했다. 이는 pixel당 정렬 비용을 피하기 위해 전체 화면을 16×16 tile로 나눈다.

렌더링 과정은 다음과 같다.

1. 각 3D Gaussian을 view frustum에 대해 culling한다. 99% confidence interval 밖에 있는 Gaussian을 제거하고, 가까운 near plane이나 view frustum 밖의 Gaussian도 reject한다.
...
- 저해상도에서 시작하는 warm-up을 사용한다. 초기에는 이미지 해상도를 1/4로 낮추어 최적화를 시작하고, 250 및 500 iteration에서 두 번 upsampling하여 최종 해상도로 올린다.
...
Mip-NeRF360, Tanks&Temples, Deep Blending 데이터셋에서 실험한 결과, 본 방법은 Mip-NeRF360과 동등하거나 더 나은 quality를 달성하면서, 학습 시간은 InstantNGP나 Plenoxels 수준으로 빠르다. 특히 real-time(≥30fps, 최대 134fps) 렌더링을 1080p에서 달성한 것은 이전 어떤 방법도 하지 못한 것이다.

![Table1](/assets/img/3dgs/table1_quantitative.png)
_Table 1. Mip-NeRF360, Tanks&Temples, Deep Blending 세 데이터셋에서 Plenoxels, InstantNGP(Base/Big), Mip-NeRF360, Ours(7K/30K)를 SSIM·PSNR·LPIPS와 학습 시간·FPS·메모리로 정량 비교한 표._

표에서 보듯, Plenoxels와 InstantNGP는 학습이 매우 빠르지만 unbounded 장면의 품질(SSIM/PSNR/LPIPS)에서 Mip-NeRF360에 뒤진다. 반대로 Mip-NeRF360은 최고 품질을 내지만 학습에 최대 48시간이 걸리고 real-time 렌더링이 불가능하다. 본 방법(Ours-30K)은 Mip-NeRF360급 품질을 수십 분의 학습으로 달성하면서, 동시에 실시간 FPS로 렌더링한다. 대표적으로 Mip-NeRF360 데이터셋에서 Ours-30K는 SSIM 0.815, PSNR 27.21을 기록하며 평균 134fps로 렌더링한다. 짧게 학습한 Ours-7K도 이미 경쟁력 있는 품질을 보인다.
...
![Table2](/assets/img/3dgs/table2_ablation.png)
_Table 2. 주요 설계 요소(SfM 초기화, densification, anisotropic covariance, gradient depth 제한 등)를 하나씩 바꾸거나 제거했을 때의 품질 변화를 정리한 ablation 결과._
...
### Limitations
- 잘 관측되지 않은(sparsely observed) 영역에서는 artifact가 남을 수 있다. 이는 다른 방법(Mip-NeRF360 등)도 공통으로 겪는 문제다.
- Gaussian이 지나치게 커지는 경우(특히 view-dependent appearance가 있을 때) popping artifact가 생길 수 있다. 그 원인 중 하나는 rasterizer가 guard band를 이용해 Gaussian을 trivial하게 reject하는 방식에 있다.
- 현재 어떤 형태의 regularization도 사용하지 않는다. Regularization을 도입하면 미관측 영역의 artifact와 popping 문제를 모두 완화할 수 있을 것으로 기대된다.
- 대부분 동일한 하이퍼파라미터로 동작하지만, 일부 까다로운 장면(예: position learning rate에 민감한 경우)에서는 조정이 필요할 수 있다.
- 메모리 사용량이 크다. 완전한 장면을 표현하려면 수백만 개의 Gaussian이 필요하며, 학습 시 peak GPU 메모리가 약 20GB에 이르러 NeRF 기반 방법보다 부담이 크다.

## Conclusion

본 논문은 3D Gaussian을 명시적(explicit) scene representation으로 사용하고, adaptive density control과 tile-based differentiable rasterizer를 결합하여, radiance field의 품질을 유지하면서도 real-time rendering을 최초로 달성했다. 이는 이후 수많은 후속 연구(dynamic scene, compression, editing 등)의 기반이 된 중요한 연구이다.
